...
`yieldContinuation`가 호출하는 `Continuation.yield`는 JNI(Java Native Interface)를 통해 JVM과 연결된 네이티브 메서드인 `doYield`를 통해 스레드를 일시 중단 시킵니다.
...
다시 가상 스레드가 실행되면 `unpark` 메서드가 호출되어 유저 스레드에 마운트되고 컨텍스트가 로딩됩니다. `unpark`는 `submitRunContinuation`를 호출하여 상태를 `RUNNING`으로 변경하고 등록된 작업을 다시 시작합니다. 만약 상태가 `PINNED`이면 이미 스레드가 마운트되어 있으므로 이미 마운트된 스레드를 재개합니다.
