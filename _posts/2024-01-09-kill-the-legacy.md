...
더럽고 꼴보기 싫은 기존 서비스를 시원하게 날려버리고, 우아하게 새 서비스를 배포합니다. 서비스는 어떤 버그도 없이 원활하게 동작하기 시작하죠. 드디어, 우리는 기존 서비스와 작별하게 되었습니다!
...
기존 서비스가 조직에게 제공하는 가치와 안정성을 고려해보며 정말로 바꿔야 하는지 다시 한 번 신중히 고민해봅시다. 더 효율적이고 품질 좋게 만들 수 있을까요?
...
물론 가지고 있는 신뢰 자산을 넘어서 더 많은 변화를 일으킬 수 있습니다. 이는 마치 신뢰를 대출하여 부채를 갖는 것과 비슷합니다. 그러나 대출을 하기 위해서는 이미 최소한의 신뢰 자산이나 대채할 수 있는 담보를 보유하고 있어야 합니다.
...
새로운 서비스가 안정적으로 동작하는지 확인하기 위해 기존 서비스와 기능을 비교하며 테스트해야 합니다.

기존 서비스에서 사용자 요청을 수집하여 실제 데이터 분포와 유사한 테스트 데이터를 구축하세요. 그리고 새 서비스가 동일하게 동직하는지 확인해 봅시다.
...
점진적으로 새로운 서비스로 오는 트래픽을 늘려가며, 마침내는 기존 서비스의 기능을 완전히 대체합니다. 모든 요청이 새 서비스에서 원활하게 처리된다는 확신이 들면, 기존 서비스의 기능을 안전하게 제거할 수 있습니다.
