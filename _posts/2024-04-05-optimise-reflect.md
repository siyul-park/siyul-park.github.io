---
title: "컴파일을 통해 리플렉션을 줄여보자"
date: 2024-04-05 12:00:00 +0900
categories: "system"
tags: ["system", "memory", "golang"]
---

런타임 시점에 객체를 유연하게 처리하기 위해 리플렉션은 인코딩 라이브러리에서 자주 사용됩니다. 더 빠른 성능을 얻기 위해서는 자동 코드 생성이나 전용 인터페이스를 활용하는 것이 좋지만, 이는 추가적인 코드 관리를 필요로 하기 때문에 간편한 리플렉션을 선호하는 경우도 많습니다.

[uniflow](https://github.com/siyul-park/uniflow)에서는 여러 연산 단위인 노드간에 데이터를 자유롭게 전달 받기 위해 JSON과 유사한 타입을 새롭게 정의해서 활용합니다. 이 타입은 여러 노드에서 동시에 사용해도 문제가 없도록 불변하게 설계되었습니다.
...
외부 라이브러리와 통신을 위해 종종 다른 구조체나 표준 객체로 변환되어야 합니다. 이런 변환 과정에서 유연하게 모든 타입에 대응하기 위해 리플렉션을 활용하여 타입에 대한 정보를 런타임에 처리합니다.
...
그러나 객체를 변환하는 리플렉션 작업은 많은 자원을 소모합니다. `GET /ping` 요청에 대한 응답으로 `pong`을 반환하는 서버에서는 객체 변환 작업은 전체 CPU 자원의 27%를 소모했습니다.
...
성능 문제를 해결하기 위해 리플렉션을 대체하는 더 효율적인 방법을 찾는 중, `encoding/json`과 호환되면서도 높은 성능을 보이는 라이브러리인 [go-json](https://github.com/goccy/go-json)을 발견했습니다. 이 라이브러리는 버퍼 재사용, 컴파일 과정을 통한 리플렉션 최적화, 커스텀 리플렉션 라이브러리를 활용한 이스케이프 회피 등 다양한 방법을 사용하여 성능을 향상시킵니다. 특히, 컴파일 과정을 통한 리플렉션 최적화는 매우 효과적인 해결책으로 보였습니다.

이 방법은 타입 정보를 활용하여 각 타입별로 최소한으로 리플렉션이 사용되는 변환 프로세스를 컴파일합니다. 이를 효율적으로 활용하여 각 필드들의 주소에 복사하거나 리플렉션 없이 직접 접근하는 방식으로 변환 프로세스를 구현합니다. 컴파일 과정은 각 타입별로 단 한 번만 일어나며, 컴파일 결과로 생성되는 변환 프로세스는 리플렉션 사용을 최소화하여 빠른 속도를 보장합니다.
...
성능 최적화 이후에는 최대 3.25배까지 성능이 향상된 놀라운 결과를 얻었습니다. 그러나 타입을 미리 추론하기 어려운 `map`과 같은 경우에는 컴파일 단계에서 약간의 오버헤드가 발생하여 성능이 소폭 감소했습니다.
